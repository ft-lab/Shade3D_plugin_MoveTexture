 ...
 * UVテクスチャのShader.
 */
#ifndef _UVTEXTURESHADER_H
#define _UVTEXTURESHADER_H

#include "GlobalHeader.h"

#define UVTEXTURE_SHADER_MAGIC_NUMBER 0x87ac798b

class CUVTextureShaderInterface : public sxsdk::shader_interface
{
private:
	sxsdk::shade_interface &m_shade;
	CMoveTextureUVShaderData m_data;				// ダイアログボックスでの設定で使うデータ.
	...
	public:
		...
		ShaderInfoC (CMoveTextureUVShaderData& data) : data(data) { }
		unsigned magic_number () const { return UVTEXTURE_SHADER_MAGIC_NUMBER; }		// 適当な数値.
	};
...
private:
	virtual sx::uuid_class get_uuid (void *) { return UVTEXTURE_SHADER_ID; }
...
public:
	CUVTextureShaderInterface (sxsdk::shade_interface& _shade);
	virtual ~CUVTextureShaderInterface ();

	/**
	 * プラグイン名.
	 */
	static const char *name (sxsdk::shade_interface *_shade) {
		return _shade->gettext("uvTexture_shader_title");
	}
};
